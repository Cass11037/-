...
def create_connection(host_name, user_name, user_password, db_name):
    ...
    return connection
connection = create_connection("localhost", "user1", "Okeeeyfran11037!", "steam_new")
...
CREATE TABLE IF NOT EXISTS ratings (
  ...
  date DATE NOT NULL,
  FOREIGN KEY fr_player_id (player_id) REFERENCES users(id),
  FOREIGN KEY fr_game_id (game_id) REFERENCES games(id),
  PRIMARY KEY (id)
) ENGINE = InnoDB
"""
...
CREATE TABLE IF NOT EXISTS comments (
  ...
  FOREIGN KEY fk_player_id (player_id) REFERENCES users(id),
...
#используем JOIN - Объединяем вывод двух таблиц
...
"""
#avg - средняя оценка для каждой игры определенного жанра, avarage_rating - новое название столбца
t_ = execute_read_query(connection, selected_with_where)
